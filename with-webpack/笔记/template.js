window.loadComponent = (function () {
    ...
    const registerComponent = ({ template, style, name, listeners }) => {
        class UnityComponent extends HTMLElement {
            connectedCallback() {
                ...
                this._attachListeners(template.content.children);
                this._upcast();
            }
            _attachListeners(children) {
                for (var item of children) {
                    initEvents(item, item.getAttribute('@click'), listeners)
                    this._attachListeners(item.children)
                }
            }
            _upcast() {
                // const shadow = this.attachShadow({ mode: 'open' });
                // shadow.appendChild(document.importNode(template.content, true));
                const root = this.host.createShadowRoot();
    ...
    const initEvents = (node, nodeEvent, listeners) => {
        if (nodeEvent) {
            console.log(node)
            const listener = listeners[nodeEvent.toLowerCase()]
    ...
    const getListeners = (settings) => {
        return Object.entries(settings).reduce((listeners, [setting, value]) => {
            ...
            // }
            listeners[setting.toLowerCase()] = value
            return listeners;
        }, {});
    }
    ...
    const getSettings = ({ template, style, script }) => {
        ...
        return import(jsURL).then((module) => {
            const listeners = getListeners(module.default.method);
            return {
